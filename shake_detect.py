...
# --------------------------------------------
X_train = []
listing1 = os.listdir('/home/alex/Desktop/csce636/dataset/RGB/train/')
listing1.sort()
listing1.sort(key=lambda x: int(x[:-4]))
print("Enter training Videos directory")

for vid in listing1:
    vid = '/home/alex/Desktop/csce636/dataset/RGB/train/' + vid
    train_frames = []
...
model.add(layers.Conv3D(32, (2, 3, 3), strides=(2, 1, 1), padding='same', activation='relu', input_shape=(20, 320, 180, 1)))
...
results = results.reshape((5, 6))
print(results)
a = model.evaluate(X_test_array, test_labels)
print(a)
model.save("/home/alex/Desktop/csce636/dataset/model/my_model.h5")
for k in range(5):
    json_data = {"Tremor": []}
    for i in range(6):
        json_data["Tremor"].append([i, int(results[k][i])])
    json_path = "/home/alex/Desktop/csce636/dataset/json/test" + str(k) + ".json"
